...
import {
  ...
  LayoutDashboard, Shield, AlertCircle, Loader2, RefreshCw,
  ChevronDown, Eye, EyeOff, Mail, User, Crown, LogOut,
} from "lucide-react";
...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505]">
      {/* Header */}
      <header className="border-b border-[#1a1a1a] bg-[#050505]/95 backdrop-blur-sm sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-1.5 bg-[#39FF14]/10 border border-[#39FF14]/20 rounded-lg">
              <Shield size={16} className="text-[#39FF14]" />
            </div>
            <div>
              <span className="text-white font-bold text-sm">Painel Admin</span>
              <p className="text-gray-600 text-[10px] font-mono">DashAds Pro</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/dashboard"
              className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-[#39FF14] transition-colors px-3 py-1.5 rounded-lg border border-[#1a1a1a] hover:border-[#39FF14]/20">
              <LayoutDashboard size={12} /> Dashboard
            </Link>
            <button
              onClick={() => fetch("/api/auth/logout", { method: "POST" }).then(() => { window.location.href = "/login"; })}
              className="flex items-center gap-1.5 text-xs text-gray-600 hover:text-rose-400 transition-colors px-3 py-1.5 rounded-lg border border-[#1a1a1a] cursor-pointer">
              <LogOut size={12} /> Sair
            </button>
          </div>
        </div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="bg-[#111] border border-[#1a1a1a] text-white placeholder-gray-700 rounded-xl pl-8 pr-4 py-2 text-xs focus:outline-none focus:border-[#39FF14]/40 w-40 sm:w-56 transition-all font-mono"
                />
              </div>
              <button onClick={load} className="p-2 rounded-xl bg-[#111] border border-[#1a1a1a] text-gray-500 hover:text-[#39FF14] transition-all cursor-pointer">
                <RefreshCw size={13} className={loading ? "animate-spin" : ""} />
              </button>
              <button onClick={openCreate}
                className="flex items-center gap-1.5 text-xs font-semibold bg-[#39FF14] hover:bg-[#2bcc10] text-black px-3 py-2 rounded-xl transition-all cursor-pointer shadow-lg shadow-[#39FF14]/10">
                      ...
                      <td className="px-4 sm:px-6 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEdit(u)} title="Editar"
                            className="p-1.5 rounded-lg text-gray-600 hover:text-[#39FF14] hover:bg-[#39FF14]/10 transition-all cursor-pointer">
                            <Edit2 size={13} />
                          </button>
                          ...
                          <button onClick={() => openDelete(u)} title="Excluir"
                            className="p-1.5 rounded-lg text-gray-600 hover:text-rose-400 hover:bg-rose-500/10 transition-all cursor-pointer">
                            <Trash2 size={13} />
                          </button>
                        </div>
                      </td>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={closeModal}>
          ...
          <div
            className="relative w-full sm:max-w-md bg-[#0a0a0a] border border-[#1a1a1a] rounded-t-3xl sm:rounded-2xl shadow-2xl shadow-black/80 overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[#111]">
              ...
              <button onClick={closeModal} className="text-gray-600 hover:text-white transition-colors cursor-pointer">
                <X size={18} />
              </button>
            </div>
                  ...
                  <Field label="Senha" icon={Key}>
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => setShowPass(!showPass)}
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-300 cursor-pointer">
